...
static inline constexpr std::chrono::hours operator "" _h(unsigned long long hours){
...
static inline constexpr uint64_t operator "" _T(unsigned long long giga){
  return giga * Xi::pow(10, 9);
}

static inline constexpr uint64_t operator "" _G(unsigned long long tera){
