...
std::string getInputAndWorkInBackground(const std::vector<T> &availableCommands, std::string prompt,
  ...
  if (backgroundRefresh) {
    ...
    std::future<std::string> inputGetter =
        std::async(std::launch::async, [&availableCommands, &prompt] { return getInput(availableCommands, prompt); });
...
template <typename T>
std::string getInput(const std::vector<T> &availableCommands, std::string prompt) {
