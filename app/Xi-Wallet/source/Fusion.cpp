...
size_t makeFusionTransaction(CryptoNote::WalletGreen &wallet, uint64_t threshold, uint32_t height) {
...
void fullOptimize(CryptoNote::WalletGreen &wallet, uint32_t height) {
...
bool optimize(CryptoNote::WalletGreen &wallet, uint64_t threshold, uint32_t height) {
  ...
  while (true) {
    ...
    if (tmpFusionTxID == CryptoNote::WALLET_INVALID_TRANSACTION_ID) {
      ...
      if (fusionTransactionHashes.size() == 1) {
        ...
        std::cout << SuccessMsg("Created " + std::to_string(fusionTransactionHashes.size()) + " fusion transactions!")
                  << std::endl;
      }
    }
  }
    ...
    for (const auto &tx : fusionTransactionHashes) {
      ...
      if (std::find(unconfirmedTxHashes.begin(), unconfirmedTxHashes.end(), tx) != unconfirmedTxHashes.end()) {
        ...
        /* We can't find this transaction in the unconfirmed
           ...
           output message */
        fusionTransactionHashes.erase(std::remove(fusionTransactionHashes.begin(), fusionTransactionHashes.end(), tx),
...
bool fusionTX(CryptoNote::WalletGreen &wallet, CryptoNote::TransactionParameters p, uint32_t height) {
  std::cout << WarningMsg(
