    ...
    if (m_currency.upgradeHeight(m_targetVersion) != UNDEF_HEIGHT) {
      ...
      if (m_blockchain.size() <= upgradeHeight()) {
        ...
        if (m_blockchain.size() % (60 * 60 / m_currency.difficultyTarget()) == 0) {
          ...
          logger(Logging::TRACE, Logging::BRIGHT_BLUE)
              << "###### UPGRADE is going to happen after block index " << upgradeHeight() << " at about "
              << upgradeTimeStr << " (in " << Common::timeIntervalToString(interval) << ")! Current last block index "
              << (m_blockchain.size() - 1) << ", hash " << cachedBlock.getBlockHash();
        }
