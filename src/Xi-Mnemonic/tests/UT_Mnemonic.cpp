...
#define XI_TEST_SUITE Xi_Mnemonic

namespace {
std::map<std::string, std::string> englishBip39{
    {{
     ...
     },
     {
         ...
         "scissors invite lock maple supreme raw rapid void congress muscle digital elegant little brisk hair mango "
         "congress clump",
     },
     {
...
TEST(XI_TEST_SUITE, Bip39) {
  using namespace Xi;
  using namespace Xi::Mnemonic;

  for (const auto& pair : englishBip39) {
    const auto data = Common::fromHex(pair.first);
    const auto encoding = encode(data, WordList::English);

    ASSERT_FALSE(encoding.isError());
    EXPECT_EQ(*encoding, pair.second);

    const auto decoding = decode(*encoding, WordList::English);

    ASSERT_FALSE(decoding.isError());
    EXPECT_EQ(data, *decoding);
  }
}
