...
std::string formatDollars(const uint64_t amount) {
  /* We want to format our number with comma separators so it's easier to
     ...
     such as ja_JP.utf8, 1 XI will actually be formatted as 100 XI, which
...
std::string createIntegratedAddress(const std::string &address, const std::string &paymentID) {
  ...
  return Tools::Base58::encode_addr(CryptoNote::Config::CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX, paymentID + keys);
}
