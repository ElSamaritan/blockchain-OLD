...
void (*const extra_hashes[4])(const void *, size_t, char *) = {Crypto::hash_extra_blake, Crypto::hash_extra_groestl,
                                                               Crypto::hash_extra_jh, Crypto::hash_extra_skein};

/*!
 * \attention Randomizer::size must be a multiple of 32
 * \attention Length must be at least 32 byte.
 */
void init_randomizer(const void *data, size_t length, Crypto::CN_ADAPTIVE_RandomValues *randomizer,
                     Crypto::Hash &hash) {
  ...
  // We assume the hash has been already initialized with some values from the caller, as well as the seed, for initial
  // run.
  for (uint32_t i = 0; i < randomizer->size / 32; ++i) {
    const uint32_t offset = i * 32;

    // First: Pick some random operatos using keccak1600, note since we are using __Data they depend on the previous
    // iteration
    std::memcpy(randomizer->operators + offset, &hash, 32);
    randomizer->operationsIndex = hash.data[0];

    // Second: Make the values of the randomizer dependent on the last iterations hashPtr, calculated lastly. For the
    // first loop we simply have chosen cn_fast_hash.
    ...
    for (uint32_t j = 2; j < 32; ++j) {
      cn_adaptive_apply_operator(reinterpret_cast<uint8_t *>(randomizer->values) + offset + j,
    ...
    // Third: We need to initialize the indices, we will not bother for now whether they encode legit addresses, the
    // cn_adaptive algorithm will take care of this within the implementation itself.
    ...
    *(reinterpret_cast<int8_t *>(randomizer->indices + offset) + 2) = *(randomizer->values + offset + 29);
    ...
    for (uint16_t j = 0; j < length; ++j) {
      ...
      cn_adaptive_apply_operator(reinterpret_cast<uint8_t *>(__Data.data()) + j,
                                 randomizer->values + offset + (randomizer->indices[rI] % 32),
                                 randomizer->operationsIndex, randomizer->operators + rI, 1);
    }

    // Lastly: In order to make every iteration dependent on the previous we will update our hash to yield the new
    // operators for the next iteration.
...
void init_salt(char *salt, size_t saltLength, Crypto::Hash &hash, Crypto::CN_ADAPTIVE_RandomValues *randomizer) {
  ...
  for (size_t i = 2; i < saltLength; ++i) {
    ...
    // In order to make every byte very depedent to be evaulated in the sequence given we will alter the randomizer
    // using the previous byte itself, somehow like the scratchpad alters itself.
...
void Crypto::CNX::Hash_v0::operator()(const void *data, size_t length, Crypto::Hash &hash) const {
  ...
  for (std::size_t i = 0; accumulatedScratchpad < 7 * 256_kB; ++i) {
    ...
    const uint32_t seed = *reinterpret_cast<uint32_t *>(&hash);
    ...
    auto xx = static_cast<uint16_t>(seed % __Twister->next(5, 80) + __Twister->next(5, 80));
    auto yy = static_cast<uint16_t>(seed % __Twister->next(5, 80) + __Twister->next(5, 80));
    auto zz = static_cast<uint16_t>(seed % __Twister->next(5, 80) + __Twister->next(5, 80));
