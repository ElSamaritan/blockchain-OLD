...
#define CRYPTO_MAKE_COMPARABLE(type)                                                                              \
  namespace Crypto {                                                                                              \
  inline bool operator==(const type &_v1, const type &_v2) { return std::memcmp(&_v1, &_v2, sizeof(type)) == 0; } \
  inline bool operator!=(const type &_v1, const type &_v2) { return std::memcmp(&_v1, &_v2, sizeof(type)) != 0; } \
  }
...
#define CRYPTO_MAKE_HASHABLE(type)                                                                    \
  ...
  struct hash<Crypto::type> {                                                                         \
    size_t operator()(const Crypto::type &_v) const { return reinterpret_cast<const size_t &>(_v); }  \
  };                                                                                                  \
  }
