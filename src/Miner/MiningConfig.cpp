...
void MiningConfig::parse(int argc, char** argv) {
  ...
  options.add_options("Core")("help", "Display this help message", cxxopts::value<bool>(help)->implicit_value("true"))(
  ...
  options.add_options("Daemon")("daemon-address",
                                "The daemon [host:port] combination to use for node operations. This option overrides "
                                "--daemon-host and --daemon-rpc-port",
      ...
      cxxopts::value<size_t>(scanPeriod)->default_value("30"), "#");
  ...
  options.add_options("Mining")("address", "The valid CryptoNote miner's address",
      ...
      "Timestamp incremental step for each subsequent block. May be set only if --first-block-timestamp has been set.",
