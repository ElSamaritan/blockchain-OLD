...
// Copyright (c) 2018, The Galaxia Project
//
...
#define ADPATIVE_post_aes(sh, v22)                         \
  ...
  b[1] = p[1];                                             \
  __mul();                                                 \
  if (v22) ADPATIVE_VARIANT2_2();                          \
  if (sh) ADPATIVE_VARIANT2_SHUFFLE_ADD_SSE2(hp_state, j); \
  a[0] += hi;                                              \
  a[1] += lo;                                              \
  p = U64(&hp_state[j]);                                   \
  p[0] = a[0];                                             \
...
void cn_slow_hash(const void *data, size_t length, char *hash, int light, int variant, int prehashed,
  ...
  hp_state = xi_hash_allocate_state(PAGE_SIZE);
  ...
  U64(a)[0] = U64(&state.data.k[0])[0] ^ U64(&state.data.k[32])[0];
  U64(a)[1] = U64(&state.data.k[0])[1] ^ U64(&state.data.k[32])[1];
  U64(b)[0] = U64(&state.data.k[16])[0] ^ U64(&state.data.k[48])[0];
  U64(b)[1] = U64(&state.data.k[16])[1] ^ U64(&state.data.k[48])[1];
  ...
  hash_extra_functions[state.hs.b[0] & 3](&state, 200, hash);
}

void cn_adaptive_apply_operator(uint8_t *inPlaceOperand, const int8_t *appliedOperand, uint8_t operationsIndex,
                                uint8_t *operation, uint32_t size) {
  void (*const fn)(uint8_t * inPlaceOperand, const int8_t *appliedOperand, uint8_t op) =
      CN_ADAPTIVE_OP_LOOKUP[operationsIndex];
  for (uint32_t i = 0; i < size; ++i) fn(inPlaceOperand + i, appliedOperand + i, operation[i]);
}

void cn_adaptive_randomize_scratchpad(CN_ADAPTIVE_RandomValues *r, char *salt, uint8_t *scratchpad, uint32_t memory,
                                      uint32_t variant) {
  ...
  for (uint32_t i = 0; i < memory / r->size / 2; ++i) {
    uint8_t operationsIndex = r->operationsIndex;
    void (*const fn)(uint8_t * inPlaceOperand, const int8_t *appliedOperand, uint8_t op) =
        CN_ADAPTIVE_OP_LOOKUP[operationsIndex];
    for (uint32_t j = 0; j < r->size; ++j) {
      const uint32_t scratchpadIndex = r->indices[j] % memory;
      fn(scratchpad + scratchpadIndex, r->values + j, r->operators[j]);
      operationsIndex ^= scratchpad[scratchpadIndex];
    }
    r->operationsIndex = operationsIndex;
  }
}
...
void cn_adaptive_slow_hash(const void *data, size_t length, char *hash, int variant, int prehashed, size_t rand_iters,
  ...
  hp_state = xi_hash_allocate_state(memory);
...
STATIC void cn_mul128(const uint32_t *aa, const uint32_t *bb, uint32_t *r) {
  ...
  __asm__ __volatile__(
      ...
      "str   %[t0], %[hl]\n\t"
      "str   %[t1], %[hh]\n\t"
      : [t0] "=&r"(t0), [t1] "=&r"(t1), [t2] "+r"(t2), [t3] "+r"(t3), [hl] "=m"(r[0]), [hh] "=m"(r[1]), [ll] "=m"(r[2]),
        [lh] "=m"(r[3])
