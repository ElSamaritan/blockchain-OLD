...
std::tuple<std::string, Crypto::SecretKey> MnemonicToPrivateKey(std::vector<std::string> words) {
  ...
  for (auto &word : words) {
    ...
    std::transform(word.begin(), word.end(), word.begin(), [&loc](char c) { return std::tolower(c, loc); });
  ...
  if (!HasValidChecksum(words)) {
    return std::make_tuple("Mnemonic seed has incorrect checksum!", key);
  }
  ...
  for (size_t i = 0; i < words.size() - 1; i += 3) {
    ...
    const uint32_t val = w1 + wlLen * (((wlLen - w1) + w2) % wlLen) + wlLen * wlLen * (((wlLen - w2) + w3) % wlLen);
