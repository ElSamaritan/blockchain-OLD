...
bool checkNodeStatus(CryptoNote::INode &node) {
  while (node.getLastKnownBlockHeight() == 0) {
    ...
    msg << "It looks like " << WalletConfig::daemonName << " isn't open!" << std::endl
        ...
        << "Alternatively, perhaps " << WalletConfig::daemonName << " can't communicate with any peers." << std::endl
        << std::endl
...
template void printCommands(const std::vector<Command> &commands, int offset);

template void printCommands(const std::vector<AdvancedCommand> &commands, int offset);
