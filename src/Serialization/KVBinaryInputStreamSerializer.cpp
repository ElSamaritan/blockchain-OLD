...
bool KVBinaryInputStreamSerializer::binary(void* _value, size_t size, Common::StringView name) {
  std::string str;
  ...
  memcpy(_value, str.data(), size);
  return true;
}

bool KVBinaryInputStreamSerializer::binary(std::string& _value, Common::StringView name) {
  return (*this)(_value, name);  // load as string
}
