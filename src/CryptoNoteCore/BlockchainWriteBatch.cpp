...
BlockchainWriteBatch& BlockchainWriteBatch::insertCachedTransaction(const ExtendedTransactionInfo& transaction,
  ...
  rawDataToInsert.emplace_back(DB::serialize(DB::TRANSACTIONS_COUNT_KEY, DB::noKey(), totalTxsCount));
  return *this;
}
...
BlockchainWriteBatch& BlockchainWriteBatch::insertCachedBlock(const CachedBlockInfo& block, uint32_t blockIndex,
  ...
  rawDataToInsert.emplace_back(DB::serialize(DB::LAST_BLOCK_INDEX_KEY, DB::noKey(), blockIndex));
  return *this;
}
...
BlockchainWriteBatch& BlockchainWriteBatch::insertKeyOutputAmounts(const std::set<IBlockchainCache::Amount>& amounts,
  ...
  rawDataToInsert.emplace_back(
      DB::serialize(DB::KEY_OUTPUT_AMOUNTS_COUNT_KEY, DB::noKey(), totalKeyOutputAmountsCount));
...
BlockchainWriteBatch& BlockchainWriteBatch::removeCachedBlock(const Crypto::Hash& blockHash, uint32_t blockIndex) {
  ...
  rawDataToInsert.emplace_back(DB::serialize(DB::LAST_BLOCK_INDEX_KEY, DB::noKey(), blockIndex - 1));
  return *this;
}
