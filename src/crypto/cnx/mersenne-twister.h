...
class MersenneTwister {
 ...
 public:
  MersenneTwister(uint32_t seed) { set_seed(seed); }
  ...
  void next_bytes(uint8_t* data, uint32_t length) {
    for (uint32_t i = 0; i < length; i++) data[i] = (uint8_t)(generate_uint() / (0xffffffff / (uint32_t)0xff));
  }

  void next_bytes(char* data, uint32_t length) {
    for (uint32_t i = 0; i < length; i++) data[i] = (char)(generate_uint() / (0xffffffff / (uint32_t)0xff));
  }
};
