...
void blake224_init(blake_state *S) {
  S->h[0] = 0xC1059ED8;
  S->h[1] = 0x367CD507;
  S->h[2] = 0x3070DD17;
  S->h[3] = 0xF70E5939;
...
void blake256_update(blake_state *S, const uint8_t *data, uint64_t datalen) {
  int left = S->buflen >> 3;
  int fill = 64 - left;
...
void hmac_blake256_init(blake_hmac_state *S, const uint8_t *_key, uint64_t keylen) {
...
// keylen = number of bytes
void hmac_blake224_init(blake_hmac_state *S, const uint8_t *_key, uint64_t keylen) {
  const uint8_t *key = _key;
  uint8_t keyhash[32];
  uint8_t pad[64];
  uint64_t i;
...
// datalen = number of bits
void hmac_blake256_update(blake_hmac_state *S, const uint8_t *data, uint64_t datalen) {
  // update the inner state
  blake256_update(&S->inner, data, datalen);
}

// datalen = number of bits
void hmac_blake224_update(blake_hmac_state *S, const uint8_t *data, uint64_t datalen) {
  // update the inner state
  blake224_update(&S->inner, data, datalen);
}

void hmac_blake256_final(blake_hmac_state *S, uint8_t *digest) {
  uint8_t ihash[32];
...
void hmac_blake224_final(blake_hmac_state *S, uint8_t *digest) {
  uint8_t ihash[32];
