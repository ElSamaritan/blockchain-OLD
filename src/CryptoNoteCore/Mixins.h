...
class Mixins {
 public:
  ...
  static std::tuple<uint64_t, uint64_t> getMixinAllowableRange(const uint32_t height) {
    ...
    if (height >= CryptoNote::Config::MIXIN_LIMITS_V3_HEIGHT) {
      minMixin = CryptoNote::Config::MINIMUM_MIXIN_V3;
      maxMixin = CryptoNote::Config::MAXIMUM_MIXIN_V3;
    } else if (height >= CryptoNote::Config::MIXIN_LIMITS_V2_HEIGHT) {
      minMixin = CryptoNote::Config::MINIMUM_MIXIN_V2;
      maxMixin = CryptoNote::Config::MAXIMUM_MIXIN_V2;
    } else if (height >= CryptoNote::Config::MIXIN_LIMITS_V1_HEIGHT) {
      minMixin = CryptoNote::Config::MINIMUM_MIXIN_V1;
      maxMixin = CryptoNote::Config::MAXIMUM_MIXIN_V1;
    }
